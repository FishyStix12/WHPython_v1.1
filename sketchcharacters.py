...
# The Python script facilitates network interaction by allowing users to input characters and 
# multipliers, which are then combined into a shellcode pattern. This pattern includes a 
# predefined list of bad characters commonly problematic in software exploitation. The script
# establishes a TCP connection with a specified IPv4 address and port, sending the constructed
# shellcode pattern as part of a payload to a target server. It employs robust error handling
# to manage potential issues such as socket errors or invalid user inputs, ensuring reliable
# execution and effective testing for vulnerability assessment in network environments.
#################################################################################################
import sys
import socket
...
shellcode = char1 * int1 + char2 * int2 + badchars
...
try:
    ...
    print(f"Unable to establish connection with the host: {e}")
